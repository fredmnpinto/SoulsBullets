...
namespace Unity.FPS.Gameplay
{
    ...
    public class PlayerCharacterController : MonoBehaviour
    {
        ...
        public float DashAccelerationFactor = 100f;
        ...
        private bool _alreadyDoubleJumped;
        ...
        void HandleCharacterMovement()
        {
            ...
            bool isSprinting = m_InputHandler.GetSprintInputHeld();
            {
                ...
                // Moves that work both mid-air and grounded
        ...
        private void Dash()
        {
            ...
            if (SetCrouchingState(false, false))
            {
                float xDashForce = 0;
                float zDashForce = 0;
                Vector3 pInput = m_InputHandler.GetMoveInput();

                Vector3 dashForce = Vector3.zero;

                if (pInput.x != 0)
                {
                    dashForce += transform.right * pInput.x;
                }

                if (pInput.z != 0)
                {
                    dashForce += transform.forward * pInput.z;
                }

                // Cancel vertical movement and launch the character in the direction he is going
                CharacterVelocity += dashForce * DashAccelerationFactor;
            }
        }

        private bool IsCharacterMoving()
        {
            return !IsGrounded || CharacterVelocity.magnitude != 0;
        }
    }
}